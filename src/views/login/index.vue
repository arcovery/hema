<template>
  <div class="login-login-container">
    ...
    <div class="login-container" :class="{ 'sign-up-mode': isSign }">
      <div class="forms-login-container">
        <div class="signin-signup">
          <transition mode="out-in">
            <login v-if="isLogin" class="sign-in-form"></login>
            <register v-else class="sign-up-form" @login="signIn"></register>
          </transition>
        </div>
      </div>

      <div class="panels-login-container">
        <div class="panel left-panel">
          <div class="content">
            <h3>河马面面</h3>
            <p>加入我们，成为本站的一份子。</p>
            <button class="btn transparent" @click="signUp">去注册</button>
          </div>
          <img src="@/assets/img/login_banner.png" class="image" alt="" />
        </div>
        <div class="panel right-panel">
          <div class="content">
            <h3>已有帐号？</h3>
            <p>立即登录已有帐号，享受独家权益。</p>
            <button class="btn transparent" @click="signIn">去登录</button>
          </div>
          <img src="@/assets/img/login_banner.png" class="image" alt="" />
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      isSign: false,
      isLogin: true,
      value: this.$store.state.settings.switch,
    }
  },
  mounted() {},
  methods: {
    ...
    // 切换开关
    change() {
      this.$store.commit('settings/CHANGE_SWITCH')
    },
  },
}
</script>
